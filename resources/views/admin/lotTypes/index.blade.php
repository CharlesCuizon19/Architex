...
@section('title', 'Lot Categories')

@section('content')
<!-- Page Title -->
<h1 class="text-2xl font-semibold mb-6">LOT TYPES</h1>
    ...
    <table class="table-fixed w-full border-collapse" id="users-table">
        <thead>
            <tr class="bg-black text-white text-sm font-semibold">
                ...
                <th class="px-10 py-3 w-1/4">Name</th>
                <th class="text-center w-[20%]">Actions</th>
            </tr>
        </thead>
        <tbody>
            @forelse($lotTypes as $lotType)
            <tr class="border-t" id="row-{{ $lotType->id }}">
                <td class="px-10 py-3 text-center">{{ $lotType->id }}</td>
                <td class="px-10 py-3 text-center">{{ $lotType->type_name }}</td>
                <td class="px-6 py-3">
                    <div class="flex justify-center items-center gap-2">
                        <button type="button"
                            onclick="deleteCategory({{ $lotType->id }})"
...
<div id="create-modal"
    ...
    <div class="bg-white rounded-3xl shadow-lg p-8 w-[600px] max-w-full animate-fade-in">
        <h2 class="text-lg font-semibold mb-6 text-gray-800">CREATE CATEGORY</h2>

        <form id="create-form" method="POST" action="{{ route('admin.lotTypes.store') }}">
            @csrf
            <div class="mb-4">
                <label for="type_name" class="block text-sm font-medium text-gray-700">Lot Name</label>
                <input type="text" name="type_name" id="type_name"
                    class="mt-1 block w-full border rounded px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none"
                    required>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#create-form').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('admin.blogCategories.store') }}",
                method: "POST",
                data: $(this).serialize(),
                success: function(response) {
                    table.row.add([
                        `<div class="text-center">${response.id}</div>`,
                        `<div class="text-center">${response.type_name}</div>`,
                        `
                        <div class="flex justify-center items-center gap-2">
                            <button type="button"
                    ...
                    Swal.fire({
                        ...
                        text: 'Your category has been successfully saved.',
                        showConfirmButton: false,
                        timer: 2000,
