    ...
    <div x-show="activeTab === 'Sitemap'" x-transition>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <template x-if="!activeLot">
                    <div class="text-center py-60">
                        <img src="{{ asset('img/properties/property-select.png') }}"
                            alt="Select Lot"
                            class="w-32 h-32 object-contain mb-6 mx-auto">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-gray-600 mt-1" x-text="activeLot.address"></p>
                                <span class="inline-block mt-3 bg-yellow-100 text-yellow-800 text-sm font-medium px-4 py-1 rounded-full"
                                    x-text="activeLot.type"></span>
                            </div>
                            <button class="bg-[#1E4D2B] text-white text-sm font-medium px-6 py-2 rounded hover:bg-green-900 transition">
                                Floor Plan
                            </button>
                        </div>
                        ...
                        <div
                            ...
                            x-ref="carousel"
                            x-init="images = [activeLot.mainImage, ...activeLot.gallery]"
                            ...
                            <img
                                ...
                                alt="Main Image"
                                class="w-full h-80 object-cover transition duration-500 ease-in-out">

                            <!-- Left Arrow -->
                            <button
                                @click="prevImage"
                                class="absolute top-1/2 left-4 -translate-y-1/2 text-white p-2 rounded-full bg-black bg-opacity-50 hover:bg-opacity-70 transition">
                            ...
                            <button
                                @click="nextImage"
                                class="absolute top-1/2 right-4 -translate-y-1/2 text-white p-2 rounded-full bg-black bg-opacity-50 hover:bg-opacity-70 transition">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex justify-center gap-2 mt-4 overflow-x-auto">
                                <template x-for="(img, i) in images" :key="i">
                                    <img
                                        ...
                                        class="w-20 h-20 object-cover rounded cursor-pointer transition hover:opacity-80">
                                </template>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="bg-white border border-gray-200 rounded-md p-4">
                                <p class="text-sm text-gray-600 mb-1">Lot Area</p>
                        ...
                        <div>
                            ...
                            <p class="text-gray-700 leading-relaxed" x-text="activeLot.description"></p>
                        </div>

                        <button @click="resetLot()" class="text-green-800 font-semibold hover:underline">
                            ← Back to Selection
                        </button>
                    </div>
                </template>
            ...
            <div class="relative bg-white overflow-hidden shadow-md">
                <img src="{{ asset('img/properties/site-map.png') }}"
                    alt="Sitemap"
                ...
                <template x-for="(lot, index) in lots" :key="index">
                    <div
                        @click="selectLot(lot)"
                        :class="[lot.color, 'absolute w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold cursor-pointer transition transform hover:scale-110']"
                        :style="'top:' + (30 + index * 15) + '%; left:' + (35 + index * 10) + '%;'"
                        x-text="lot.id">
                    </div>
                </template>

                <button
                    @click="activeTab = 'Overview'; resetLot()"
                    class="absolute top-4 right-4 text-sm text-gray-700 hover:text-green-700 font-semibold">
                    ◀ Back to Overview
                </button>
            </div>
        </div>
