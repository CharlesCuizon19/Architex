@props(['flag' => 'FloorPlan', 'plans' => []])

<div>
    <div x-data="floorPlanGallery({{ json_encode($plans) }})" class="relative z-10 w-full max-w-6xl mx-auto">
        <!-- Main Image Container -->
        <div class="relative w-full overflow-hidden bg-gray-200 aspect-[16/9] rounded-lg shadow">
            <div class="relative w-full h-full group">
                <template x-for="(plan, index) in plans" :key="index">
                    <img
                        ...
                        class="
                            w-full h-full
                            {{ $flag === 'Amenities' ? 'object-cover' : 'object-contain' }}
                ...
                <div class="absolute inset-0 flex items-center justify-between px-6">
                    <button @click="prev" class="p-3 text-white transition rounded-full bg-black/50 hover:bg-black/70">
                        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                        </svg>
                    </button>
                    <button @click="next" class="p-3 text-white transition rounded-full bg-black/50 hover:bg-black/70">
                        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div x-show="!showThumbs"
                    ...
                    class="absolute bottom-0 z-10 grid w-full grid-cols-5 gap-3 p-4 bg-gradient-to-t from-[#002B0A]/90 to-transparent">
                    ...
                    <template x-for="(plan, index) in plans" :key="index">
                        <div
                            ...
                            <img
                                :src="plan"
                                class="object-cover w-full h-28 rounded-md">
                        </div>
                    </template>
                </div>
            </div>
